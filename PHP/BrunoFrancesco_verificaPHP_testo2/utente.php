    ...
    <?php
        ...
        if(isset($_SESSION['idUtente'])){
			...
			try {				
				...
				//$stmt->closeCursor();
				$stmt = $con->prepare("SELECT avg(eta) AS Media, min(eta) AS Min, max(eta) AS Max FROM utenti");
				$stmt->execute();
				while($record = $stmt->fetch(PDO::FETCH_ASSOC)){
					echo("Media: ");
					echo($record['Media']);
        ...
        echo("</form>");
    ?>
	<form enctype="multipart/form-data" action="scrittura.php" method="post" style="margin:5%">
		<input type="submit" name="Esporta" value="Inoltra">
	</form>
