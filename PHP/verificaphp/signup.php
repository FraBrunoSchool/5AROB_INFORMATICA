...
<body>
    ...
    <?php
			if($_GET["azione"] == "salva"){
				...
				$sql="SELECT email FROM dati_persone  WHERE email == '$usr' ";   //cerco se l'utente esiste gia'
				$ok = $con->query($sql);    //eseguo la query
				
				if($ok){   //aggiungo utente
                    $sql = "INSERT INTO dati_persone (email, pswd) VALUES('$usr','$pwd')";
                    $ok = $con->query($sql);    //eseguo la query
                    header("location:/verificaphp/index.html");
